...
local function tr_tick(tr) -- do transformer tick
    for k,v in pairs(tr.players) do
        ...
        if v and user_id ~= nil then -- for each player transforming
            ...
            local tr_inc = recipe.filler or false
            local valid_units = tr.units > 0
            if tr_inc then
                valid_units = tr.units < tr.itemtr.max_units
            end
            if valid_units and recipe then -- check units
                -- check reagents
...
local function transformers_tick()
    ...
    SetTimeout(1000,transformers_tick)
end
transformers_tick()
